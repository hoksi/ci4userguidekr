...
.. php:function:: random_string([$type = 'alnum'[, $len = 8]])
    ...
    .. warning:: **basic**, **md5**, **sha1** 유형의 문자열은 암호학적으로 안전하지 않습니다.
        따라서 이러한 유형은 암호학적인 목적이나 추측할 수 없는 반환 값을 필요로하는 목적에는 사용할 수 없습니다.
    ...
    .. note:: v4.3.3 이후로 **alpha**, **alnum** and **nozero** 유형은 ``random_byte()``\ 를 사용하며, **numeric** 유형은 ``random_int()``\ 를 사용합니다.
        이전 버전에서는 암호학적으로 안전하지 않은 ``str_shuffle()``\ 를 사용했습니다.
